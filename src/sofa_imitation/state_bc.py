...
def run_bc(batch_size: int = 2, learning_rate=lambda epoch: 1e-3 * 0.99 ** epoch, num_epoch: int = 1,
    ...
    demos = npz_to_state_transitions(path, 'LigatingLoopEnv_', num_traj)
    ...
    n_run = 1
    while True:
        bc_trainer.train(n_epochs=num_epoch, progress_bar=True, log_interval=100,)
...
def hydra_run(cfg: DictConfig):
    ...
    wandb.init(project="Imitation_Sofa", config=OmegaConf.to_container(cfg, resolve=True), settings=wandb.Settings(start_method="thread"),
               notes='increased pointcloud size, fixed nn', tags=['nn=[256,128,64,32]', f'lr={lr}', 'state'])
